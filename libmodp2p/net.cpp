...
{
    return (unsigned short)(gArgs.GetArg("-port", 8332/*Params().GetDefaultPort()*/));
}
...
CNode* CConnman::ConnectNode(CAddress addrConnect, const char *pszDest, bool fCountFailure)
{
    ...
    // Resolve
    const int default_port = 8332;//Params().GetDefaultPort();
    if (pszDest) {
...
void CConnman::ThreadDNSAddressSeed()
{
    ...
    const std::vector<std::string> vSeeds; //&vSeeds = Params().DNSSeeds();
    int found = 0;
    ...
    for (const std::string &seed : vSeeds) {
        ...
        if (HaveNameProxy()) {
            ...
            if (LookupHost(host.c_str(), vIPs, nMaxIPs, true))
            {
                ...
                {
                    int nOneDay = 24*3600;
                    CAddress addr = CAddress(CService(ip,8322 )/*Params().GetDefaultPort())*/, requiredServiceBits);
...
void CConnman::ThreadOpenConnections(const std::vector<std::string> connect)
{
    ...
    while (!interruptNet)
    {
        ...
        while (!interruptNet)
        {
            ...
            if (addr.GetPort() != 8332/*Params().GetDefaultPort() */&& nTries < 50)
                continue;
...
std::vector<AddedNodeInfo> CConnman::GetAddedNodeInfo()
{
    ...
    for (const std::string& strAddNode : lAddresses) {
        CService service(LookupNumeric(strAddNode.c_str(), /*Params().GetDefaultPort()*/8332));
        if (service.IsValid()) {
...
void CConnman::PushMessage(CNode* pnode, CSerializedNetMsg&& msg)
{
    ...
    CMessageHeader hdr( message_start/*Params().MessageStart()*/, msg.command.c_str(), nMessageSize);
